...
struct ApplicationsFolderMoveWorkflow {
    ...
    private func askToMove() -> Bool {
        let alert = NSAlert()
        alert.messageText = "Move Locus Launcher to your Applications folder?"
        alert.informativeText = location.isTranslocated
            ? """
            macOS is running Locus Launcher from a temporary read-only copy, so it can't update \
            itself or launch at login. Moving it to Applications fixes both.
            """
            : """
            Locus Launcher can't update itself or launch at login reliably from its current \
    ...
    private func reportFailure(_ error: any Error) {
        let alert = NSAlert()
        alert.messageText = "Locus Launcher couldn't move itself."
        alert.informativeText = """
        \(error.localizedDescription)

        Quit Locus Launcher and drag it into your Applications folder in Finder.
        """
